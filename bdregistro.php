...
if ($_SERVER['REQUEST_METHOD'] == 'POST')#si el metodo de envio($_SERVER['REQUEST_METHOD'])es igual a post es porque los datos ya se 												 enviaron
{
	...
	if (empty($usuario) or empty($password) or empty($password2) or empty($apellido) or empty($correo)){
		...
		try{
			...
			$conexion = new PDO('mysql:host=localhost;dbname=zapateria','root','');
	...
	if ($errores == '') {
		...
		$statement = $conexion->prepare('INSERT INTO usuarios (id,nombre,apellidos,password,correo) VALUES (null,:usuario,:apellido,:pass,:correo)');
		$statement->execute(array(':usuario' => $usuario, ':apellido' => $apellido,':pass' => $password,':correo'=>$correo));#aqui ya estamos insertando los valores
